...
First, let's examine the `fsxz-claim.yaml` file which defines a PersistentVolumeClaim requesting 5GB of storage from the `fsxz-vol-sc` storage class we created earlier:

```file
manifests/modules/fundamentals/storage/fsxz/deployment/fsxz-claim.yaml
```
...
```bash
$ kubectl apply -f ~/environment/eks-workshop/modules/fundamentals/storage/fsxz/job.yaml
...
```bash
$ kubectl exec --stdin $POD_1 -n ui -- bash -c 'curl -o /fsxz/placeholder.jpg https://placehold.co/600x400/jpg?text=EKS+Workshop\\nPlaceholder'
```
